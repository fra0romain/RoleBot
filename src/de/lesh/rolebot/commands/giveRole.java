...
public class giveRole extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent e) {
		...
		if (!(msg.getRawContent().startsWith(".r ") || msg.getRawContent().startsWith(".role")
				|| msg.getRawContent().startsWith(".R")) || e.getAuthor().isBot()) {
			return;
		}
		...
		case "beginner": {
			e.getGuild().getController()
					.modifyMemberRoles(member, Arrays.asList(beginnerID), Arrays.asList(mediumID, profiID)).queue();
		...
		case "medium": {
			e.getGuild().getController()
					.modifyMemberRoles(member, Arrays.asList(mediumID), Arrays.asList(beginnerID, profiID)).queue();
			eB.setAuthor("ROLE UPDATE >> " + user.getName(), null, user.getEffectiveAvatarUrl());
		...
		case "profi": {
			e.getGuild().getController()
					.modifyMemberRoles(member, Arrays.asList(profiID), Arrays.asList(beginnerID, mediumID)).queue();
			eB.setAuthor("ROLE UPDATE >> " + user.getName(), null, user.getEffectiveAvatarUrl());
